...
public class SimpleDotComGame {
	public static void main(String[] args){
		...
		int randomNum=(int) (Math.random() * 5);	//用随机数产生第一个的位置，然后以此制作出数组
		
		int[] locations ={randomNum,randomNum+1,randomNum+2};
		theDotCom.setLocationCells(locations);	//赋值位置
		boolean isAlive=true;	//创建出记录游戏是否继续的boolean变量，用在while循环种
		...
		while(isAlive==true){
			...
			numOfGuesses++;
			if(result.equals("kill")){	//是否击沉，若击沉，则设定isAlive为false并印出猜测次数
				isAlive=false;
